...
xk = [0 2.687077009915200e-04 -9.952478391308716e-06 9.538977027657970e+07 4.315801687260146e+02 1.294668091021959e-04];
%xk=[0 ;0];
...
for k=1:1:7000
    ...
    [tt,xx]=ode45('odefcn3',tSpan,xk,[],u);%[](no opciones),para(argumentos adicionales)
    xk=xx(length(xx),:); % A/D
    x(k,:)=xk; %th(k)=xk(1);%guarada valor de la variable de estado1
    err=ref-real(xk(5));
    u=err;%-0.1*x(2);% asignación de la señal de control
    ut(k)=u;
    errt(k)=err;
    %ut_1=[0;0];
end
...
plot(time,x(:,5),'g',time,x(:,6),'linewidth',2);
...
box(axes1,'on');% grafica el borde de los ejes al rededor de la gráfia
hold(axes1,'all'); %acotar los ejes a los limites definidos(aplicar axes1)
plot(time,x(:,2),time,x(:,3),'g','linewidth',2);
xlabel('Tiempo [s]','Interpreter','latex','FontSize',22);
ylabel('presiom [psi]','Interpreter','latex','FontSize',22);
